...
class Recepcionistas extends CI_Controller{
    ...
    public function index(){
        $this->load->model('Recepcionistas_model');
        $this->load->library('pagination');
        $opciones = array();
        $desde = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
        $opciones['per_page'] = 10;
        $opciones['num_links'] = 5;
        $opciones['base_url'] = base_url().'index.php/recepcionistas/index';
        $opciones['total_rows'] = $this->Recepcionistas_model->numeroRecepcionistas();
        ...
        $data['lista'] = $this->Recepcionistas_model->mostrarResultados($opciones['per_page'],$desde);
        $data['paginacion'] = $this->pagination->create_links();
        $this->load->view('folder_recepcionistas/cabecera_recepcionistas_view');
        $this->load->view('menu_administradores_view');
        $this->load->view('folder_recepcionistas/recepcionistas_view',$data);
        $this->load->view('footer_view');
    }
    
    public function insertar_recepcionista_view(){
        $this->load->helper('form');
        $this->load->view('folder_recepcionistas/cabecera_recepcionistas_view');
        $this->load->view('menu_administradores_view');
        $this->load->view('folder_recepcionistas/recepcionistas_insertar');
        $this->load->view('footer_view');
    }

    public function insertar_recepcionista(){
        $this->load->model('Recepcionistas_model');
        $this->reglas_validacion();
        if($this->form_validation->run()==FALSE){
            $this->insertar_recepcionista_view();
        }
        else{
            $data = array(
            ...
            $this->Recepcionistas_model->insertarRecepcionista($data);
            redirect('index.php/recepcionistas/paginar');
        }
    }
    
    public function buscar_recepcionista_view(){
        $this->load->model('Recepcionistas_model');
        $this->load->library('pagination');
        ...
        $opciones['num_links'] = 5;
        $opciones['base_url'] = base_url().'index.php/recepcionistas/index';
        $opciones['total_rows'] = $this->Recepcionistas_model->numeroRecepcionistasBusqueda($busqueda,$parametro);
        $opciones['uri_segment'] = 3;
        $opciones['first_link'] = 'Primero';
        $opciones['last_link'] = 'Ultimo';
        $opciones['next_link'] = 'Siguiente';
        $opciones['prev_link'] = 'Anterior';
        $this->pagination->initialize($opciones);
        $data['lista'] = $this->Recepcionistas_model->mostrarResultadosBusqueda($opciones['per_page'],$desde,$busqueda,$parametro);
        $data['paginacion'] = $this->pagination->create_links();
        $this->load->view('folder_recepcionistas/cabecera_recepcionistas_view');
        $this->load->view('menu_administradores_view');
        $this->load->view('folder_recepcionistas/recepcionistas_view',$data);
    ...
    public function editar_recepcionista_view(){
        $this->load->helper('form');
        ...
        $this->load->view('folder_recepcionistas/cabecera_recepcionistas_view');
        $this->load->view('menu_administradores_view');
        $this->load->view('folder_recepcionistas/recepcionistas_editar',$data);
        $this->load->view('footer_view');
    }
    
    public function editar_recepcionista(){
        $this->load->model('Recepcionistas_model');
        $data = array( 
            'nombres' => $this->input->post('nombres'),
            'apellidos' => $this->input->post('apellidos'),
            ...
            $this->Recepcionistas_model->editarRecepcionista($data,$old_id);
            redirect('index.php/recepcionistas/paginar');
    }
    
    public function editar_recepcionista_permisos_view(){
        $this->load->helper('form');
        $id = $this->uri->segment('3');
        $this->load->model('Recepcionistas_model');
        $query = $this->db->get_where("usuarios",array("id_usuarios"=>$id));
        ...
        $this->load->view('folder_recepcionistas/cabecera_recepcionistas_view');
        $this->load->view('menu_administradores_view');
        $this->load->view('folder_recepcionistas/recepcionistas_editar_permisos',$data);
        $this->load->view('footer_view');
    }
    
    public function editar_recepcionista_permisos(){
        $this->load->model('Recepcionistas_model');
        $data_per = array( 
        ...
        $old_id = $this->input->post('old_id'); 
        $this->Recepcionistas_model->editarRecepcionistaPermisos($data_per,$old_id);
        redirect('index.php/recepcionistas/paginar');
    }

    public function eliminar_recepcionista(){
        $this->load->model('Recepcionistas_model'); 
        $id = $this->uri->segment('3'); 
        $this->Recepcionistas_model->eliminarRecepcionista($id);
        redirect('index.php/recepcionistas/paginar');
    }
    ...
    public function validar_username($username){
        $this->load->model('Recepcionistas_model');
        if($this->Recepcionistas_model->existeUsername($username)){
    ...
    public function validar_celular($celular){
        $this->load->model('Recepcionistas_model');
        if($this->Recepcionistas_model->existeCelular($celular)){
    ...
    public function validar_correo($correo){
        $this->load->model('Recepcionistas_model');
        if($this->Recepcionistas_model->existeCorreo($correo)){
...
//        $section->addText('Hora: '.date("H:i:s"), $estiloFuenteTipoFechaHora);
        ...
        $section->addText('Lista de recepcionistas', $estiloFuenteLista, $estiloParrafoLista);
        //Lista de administradores
        ...
        $contenido = $this->Recepcionistas_model->obtenerResultadosExportarWord();
        //Obtenemos el numero de filas de la tabla seleccionada
        
        
        //Recorremos la tabla
        for ($row = 0; $row < count($this->Recepcionistas_model->numeroRecepcionistas()); $row++){
        ...
        $filename='Lista_Recepcionistas.docx'; //save our document as this file name
...
//        $section->addText(' ');
        ...
        $section->addText('Perfil del recepcionista', $estiloFuentePerfil, $estiloParrafoPerfil);
        //Perfil de administrador

        //Obtenemos el numero de filas de la tabla seleccionada
        $contenido = $this->Recepcionistas_model->obtenerResultadosExportarWord_ID($id);
        //Obtenemos el numero de filas de la tabla seleccionada
        ...
        foreach ($contenido as $result){
            ...
            $nomRecep = "".$result['nombres']." ".$result['apellidos']."";
        }
        //Recorremos los datos
        
        //Creamos y guardamos el documento
        $filename="Recepcionista_".$nomRecep.".docx";
//        $filename='Perfil_de_administrador.docx'; //save our document as this file name
    ...
    public function exportar_excel(){
        ...
        //Titulo Lista de recepcionistas
        $this->excel->setActiveSheetIndex(0)->setCellValue('A6','Lista de recepcionistas');
        ...
        $this->excel->getActiveSheet()->setTitle('Lista de recepcionistas')
        ...
        $this->load->database();
        $this->load->model('Recepcionistas_model');
        $recepcionistas = $this->Recepcionistas_model->obtenerResultadosExportarExcel();
        $this->excel->getActiveSheet()->fromArray($recepcionistas, null, 'A9');
        
        //Datos array
        
        //Archivo descargado
        $filename='Lista_de_recepcionistas.xls';
    ...
    public function exportar_excel_id(){
        ...
        //Titulo Perfil del recepcionista
        $this->excel->setActiveSheetIndex(0)->setCellValue('B6','Perfil del recepcionista');
        ...
        $this->excel->getActiveSheet()->setTitle('Perfil del recepcionista')
            ->getColumnDimension('B')
                ->setAutoSize(TRUE);
        $this->load->database();
        $this->load->model('Recepcionistas_model');
        $recepcionistas = $this->Recepcionistas_model->obtenerResultadosExportarExcel_ID($id);
        foreach ($recepcionistas as $result){
            $this->excel->setActiveSheetIndex(0)
        ...
        //Archivo descargado
        $filename="Recepcionista_".$nomRecep.".xls";
        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment;filename="'.$filename.'"');
        header('Cache-Control: max-age=0');
        $objWriter = PHPExcel_IOFactory::createWriter($this->excel, 'Excel5');
	...
	$data['subject']='Lista de recepcionistas'; 	 
	$data['content']= $this->Recepcionistas_model->obtenerResultadosExportarPDF();
	$this->load->view('folder_recepcionistas/recepcionistas_exportar_pdf',$data);
    }
    
    public function exportar_pdf_id(){
        $this->load->model('Recepcionistas_model');
        $id = $this->uri->segment('3');
        $data['title']='Perfil de recepcionista'; 
        $data['author']='Administrador';
	$data['subject']='Perfil de recepcionista';	 
	$data['content']= $this->Recepcionistas_model->obtenerResultadosExportarPDF_ID($id);
	$this->load->view('folder_recepcionistas/recepcionistas_exportar_pdf_id',$data);
    }
