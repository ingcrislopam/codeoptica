<?php
    if (isset($this->session->userdata['logged_in'])) {
        $id_usuarios = ($this->session->userdata['logged_in']['id_usuarios']);
...
<?php 
    ...
    foreach ($content as $results) {
        $contenido[]=array
        (
            'ID'=>$results->id_datos_pacientes,
//            'Medico'=>$results->nombresMedi,
            'Paciente'=>$results->nombresPaci,
            'Cedula'=>$results->cedula,
            'Fecha de nacimiento'=>$results->fechaPaci,
            'Sexo'=>$results->sexoPaci,
            'Ciudad'=>$results->ciudad,
            'Direccion'=>$results->direccion,
            'Correo'=>$results->correoPaci
        );
